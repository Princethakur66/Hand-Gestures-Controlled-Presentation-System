...
prototxt_path = "hand_deploy.prototxt"
prototxt_path = "models/hand_deploy.prototxt"
prototxt_path = r"C:\Users\<YourUsername>\Desktop\models\hand_deploy.prototxt"
...
prototxt_path = "hand_deploy.prototxt"  # Update with your path
if not os.path.exists(prototxt_path):
    print(f"File not found: {prototxt_path}")
...
model_prototxt = "hand_deploy.prototxt"  # Path to model architecture
model_caffemodel = "hand.caffemodel"    # Path to pre-trained weights
...
while True:
    ...
    h, w = frame.shape[:2]
    blob = cv2.dnn.blobFromImage(frame, scalefactor=1.0/255, size=(300, 300), mean=(127.5, 127.5, 127.5), swapRB=True, crop=False)
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.5:
            ...
            label = f"Hand: {confidence * 100:.2f}%"
...
prototxt_path = "hand_deploy.prototxt"  # Replace with the correct path
model_path = "hand.caffemodel"         # Replace with the correct path

# Load the pre-trained model
net = cv2.dnn.readNetFromCaffe(prototxt_path, model_path)

# Initialize webcam (or use a video file by replacing `0` with the file path)
cap = cv2.VideoCapture(0)
